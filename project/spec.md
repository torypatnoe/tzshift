...
## Changelog
- 2026-06-30: Current-time mode (`tzshift` with no args) now treats the local zone as the source, so the local row shows `←you  ←source` (AC13).
- 2026-06-22: Added `tzshift config create` (scaffold a starter config, no overwrite) and `tzshift config list` (cat the current config) (AC17). Dropped the decorative `→` line prefix from `show` output (AC10). Cross-date rows now highlight the **whole line**, not just the date, and the day-offset/highlight anchor is the **source zone** (was the local row) (AC16).
- 2026-06-22: A missing source zone is no longer an error — `tzshift 14:30` interprets the time in the host's local zone (the local/`←you` row is then also `←source`) (AC1).
- 2026-06-22: `show` drops the echo/header line; the source zone is identified inline by a `←source` marker (both `←you  ←source` when source and local coincide). The "date assumed" note moves into the per-row dates (AC1, AC2).
- 2026-06-22: `show` always includes the source zone and the host's local time (the `←you` date anchor), each added only if the roster doesn't already cover it — so the `+N`/`-N` markers always have a visible reference (AC1, AC16).
- 2026-06-22: `show` row format reordered to **label, date, time, UTC offset (`±HH:MM`)**; the three-letter zone abbreviation is dropped from output (AC1). Offset uses the same renderer as `list`.
- 2026-06-20: No-config fallback is now silent (AC7) — the first-run hint is no longer printed on every invocation; the config format and fallback are documented in `tzshift --help` instead. Malformed-config errors are unchanged.
- 2026-06-20: `tzshift list` row format is now `identifier  UTC-offset  abbreviation`; the abbreviation column is shown only for real letter codes (numeric pseudo-abbreviations like `+0530` are dropped as redundant with the offset). (AC14)
- 2026-06-20: `tzshift list` always orders east-most first (mirroring `show`); no sort option. (A `--sort` flag was briefly added and then removed the same day.) (default `name`); `--sort=offset` orders zones east-most first, mirroring `show` (AC14/AC15).
...
- **AC1 — Wall-clock + source zone.** `tzshift 14:30 Asia/Kolkata` (or an alias, `tzshift 14:30 india`; routed to `show`) prints one output row per roster entry — no echo/header line. With no source zone (`tzshift 14:30`), the time is interpreted in the **host's local zone**. Each row reads **roster label, date, local wall-clock time, then the zone's UTC offset (`±HH:MM`)** — no three-letter zone abbreviation. `show` always includes a row for **the source zone the user typed** (marked `←source`) and for **the host's local time** (marked `←you`, the day-offset anchor), each added only if the roster doesn't already cover it. When the source zone is also the local zone, the one row carries both markers (`←you  ←source`). Rows are ordered and date-marked per AC15–AC16. Example output matches the Shape's narrative block.
- **AC2 — Explicit date.** `tzshift 2026-06-09 14:30 Asia/Kolkata` uses the given date for the conversion. With no date, the conversion assumes **today**; the assumed date is conveyed by the full date shown on every output row (there is no separate "date assumed" notice — the echo/header line was removed).
...
- **AC13 — Current-time mode.** `tzshift show` (or bare `tzshift`) with no timestamp prints the current time across the roster zones. The source is the local zone, so the local row is marked `←you  ←source`.

### Reference (`list`)
- **AC14 — Zone & abbreviation listing.** `tzshift list` enumerates the IANA zone identifiers tzshift knows, and for each its current UTC offset and abbreviation (the abbreviation shown only when it is a real letter code, not a numeric `+0530`-style pseudo-abbreviation), plus the user's configured `[abbreviations]` (if any). There is **no shipped abbreviation → zone mapping** to list. Zone data is read from the IANA database **embedded in the binary** (not the host OS's zoneinfo), so output is identical across Linux/macOS/unix. Output is ordered east-most first (largest current UTC offset), matching `show`'s ordering (AC15). There is no sort option.
...
- **AC7 — No-config fallback.** With no config file present, the tool falls back to translating into **local time + UTC + America/Denver** silently — no message, no error. The config file format and this fallback are documented in `tzshift --help` (not printed on every run).
...
- **AC17 — Config management subcommands.** `tzshift config create` writes a starter `~/.config/tzshift/config.toml` (creating parent dirs) and refuses to overwrite an existing file (non-zero, message). `tzshift config list` prints the current config file to stdout, or exits non-zero with a hint if none exists.

### Output & ergonomics
- **AC10 — Human-first, pipe-safe output.** Default output is column-aligned plain text — greppable and pipeable (no decorative line prefix). Color is emitted only when stdout is a TTY (and suppressed by `--no-color`); all information conveyed by color is also conveyed by text (see AC16), so piped output loses no meaning.
...
- **AC16 — Date display & cross-date highlight.** Every output row shows its date alongside the wall-clock time. The day-offset anchor is the **source zone's date** (the `←source` row, always present per AC1); for epoch mode (no typed source) the anchor falls back to the host's local date. (Current-time mode's source is the local zone.) A row whose date differs from that anchor has its **whole line highlighted in color when stdout is a TTY** and is annotated with a persistent `+1` / `-1` (or larger) day-offset marker that survives a pipe / `--no-color`, so the marker always has a visible reference. The local row is marked `←you` so it is identifiable despite not being pinned to the top.
