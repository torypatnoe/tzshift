...
- 2026-06-20: `tzshift list` defaults to east-most-first order (mirroring `show`); `--sort=name` selects alphabetical. (Originally added as `--sort=name|offset` with `name` default; default flipped to offset the same day.) (default `name`); `--sort=offset` orders zones east-most first, mirroring `show` (AC14/AC15).
