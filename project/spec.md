...
- **AC16 — Date display & cross-date highlight.** Every output row shows its date alongside the wall-clock time. The day-offset anchor is the host's local date, shown as the `←you` row (always present per AC1). A row whose date differs from that anchor is **highlighted in color when stdout is a TTY** and is annotated with a persistent `+1` / `-1` (or larger) day-offset marker that survives a pipe / `--no-color`, so the marker always has a visible reference. The local row is marked `←you` so it is identifiable despite not being pinned to the top.
