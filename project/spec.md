...
- **AC16 — Date display & cross-date highlight.** Every output row shows its date alongside the wall-clock time. The day-offset anchor is the **source zone's date** (the `←source` row, always present per AC1); for epoch / current-time modes that have no source, the anchor falls back to the host's local date. A row whose date differs from that anchor has its **whole line highlighted in color when stdout is a TTY** and is annotated with a persistent `+1` / `-1` (or larger) day-offset marker that survives a pipe / `--no-color`, so the marker always has a visible reference. The local row is marked `←you` so it is identifiable despite not being pinned to the top.
